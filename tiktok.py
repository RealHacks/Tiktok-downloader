...
# ---------- Instagram Cookie (runtime input) ----------
# Use menu option 9 to set this at runtime.
INSTAGRAM_COOKIE = ""
...
def download_custom_link(url: str, out_dir: str = DEFAULT_OUT_DIR):
    ...
    try:
        with YoutubeDL(ydl_opts) as ydl:
            ...
            entries = result["entries"] if isinstance(result, dict) and "entries" in result else [result]
...
def download_user_videos(username: str, count: Optional[int] = None, out_base: str = DEFAULT_OUT_DIR):
    username = username.lstrip("@")
    ...
    # Open folder on Android (Termux/Pydroid)
...
# ---------- Instagram account video fetching using runtime cookie ----------
def download_instagram_videos(username: str, out_base: str = DEFAULT_OUT_DIR):
    global INSTAGRAM_COOKIE
    if not INSTAGRAM_COOKIE:
        print("⚠️ Instagram cookie not set. Use option 9 to set it first.")
        return

    username = username.lstrip("@")
    out_dir = os.path.join(out_base, f"instagram_{username}")
    os.makedirs(out_dir, exist_ok=True)

    profile_url = f"https://www.instagram.com/{username}/"

    # yt-dlp supports cookies passed as a string when cookiesfromstring is True
    ydl_opts = {
        "outtmpl": os.path.join(out_dir, "%(upload_date)s_%(id)s.%(ext)s"),
        "format": "best",
        "quiet": False,
        ...
        "cookiefile": INSTAGRAM_COOKIE,  # will be interpreted as cookie string due to cookiesfromstring=True
    }

    try:
        with YoutubeDL(ydl_opts) as ydl:
            ydl.download([profile_url])
        print(f"\n✅ Done! Instagram videos saved in: {out_dir}")
        if os.path.exists("/data/data/com.termux/files/usr/bin/am"):
            os.system(f'am start -a android.intent.action.VIEW -d "file://{out_dir}" >/dev/null 2>&1')
    except Exception as e:
        print(f"⚠️ Failed to download Instagram account @{username}: {e}")
...
def main_menu():
    ...
    while True:
        ...
        print("  8) Download all videos from Instagram account")
        print("  9) Set Instagram cookie (for account downloads)")
        print("  0) Exit\n")
        ...
        if choice == "1":
            ...
            ig_user = input("Enter Instagram username (no @): ").strip().lstrip("@")
            if ig_user:
                download_instagram_videos(ig_user)
            else:
                print("⚠️ Invalid username.")

        elif choice == "9":
            # Paste the cookie string directly, e.g. sessionid=...; csrftoken=...; ...
            cookie_input = input("Paste full Instagram cookie string (single line): ").strip()
            if cookie_input:
                INSTAGRAM_COOKIE = cookie_input
                print("✅ Instagram cookie updated (in memory).")
            else:
                print("⚠️ Invalid cookie.")
            ...
            print("⚠️ Invalid option. Enter 0–9.")
